...
대한민국 17개 시·도의 학교 e스포츠 관련 공개자료를 바탕으로 여러 시작과 연결을 발견하는 편집형 리서치 웹사이트입니다. 교육청, 지방자치단체, 학교 담당자가 전국의 흐름을 이해하고 다른 지역의 대회, 정책, 시설, 동아리 및 교육과정 사례를 원문 근거와 함께 탐색할 수 있도록 만들었습니다.
...
1. 흐름, 시작점과 대표 사례를 편집형 서사로 보여 주는 학교 e스포츠 지형도
...
- 대회·행사, 학교 기반, 공간·시설, 정책·진로·미디어의 네 가지 움직임을 데이터에서 자동 산출
- 동아리, 대회, 정책, 공간, 진로라는 사용자의 질문에서 관련 사례로 바로 이동
- 근거 수준이 높은 대표 사례 세 개를 대회·교실·지역 정책 관점에서 소개
- 교실에서 지역 무대와 지원 체계로 이어지는 과정을 표현한 전용 편집 일러스트레이션

### 전국 데이터 탐색실

- 17개 시·도와 8개 카테고리를 한 표에서 비교
- 지역 행, 카테고리 열, 개별 셀에서 해당 사례 목록으로 바로 이동
- 사례 수를 순위나 활동 규모로 오해하지 않도록 고정된 지역 순서와 해석 안내 제공
...
- 선택한 지역의 GeoJSON 행정 경계를 비대화형 참고 지도로 표시
- 지도 없이도 모든 탐색과 상세 기능을 사용할 수 있는 보조 시각화
- 지도 연산은 가능한 경우 Web Worker에서 처리
...
현재 대시보드의 외부화된 기준 데이터는 `baseline/v2/site.v2.json`이며, v3 작업 투영은 `data/site.v3.json`입니다. 기준 마이그레이션에서는 230개 사례와 17개 지역을 불변 조건으로 유지합니다.
...
│   ├── search.js              # 검색·필터 로직
│   ├── region-loader.js       # 지역 GeoJSON 로더
...
- 기준 마이그레이션의 230개 사례와 17개 지역 수를 임의로 바꾸지 마십시오.
...
- 현재 230개 사례 모두 운영 상태의 독립적인 재검증이 필요합니다.
