...
| 최근 운영 배포 | 2026-09-05 | GPT-6 Astra 검수·서명과 독립 검증을 통과한 [자동 배포](https://github.com/taehyeonglim/2026-esports-landscape/actions/runs/33969332429) |
...
`data/site.v3.json`의 원본 235개 레코드 중 `src/record-scope.js`가 분리한 75개 사례의 집계입니다. 160개 지역 보조 참고 자료는 사례 집계에 포함하지 않습니다.
...
| 공개 사례 | 75건 |
| 사례 출처 레코드 | 75건 |
| 지역 범위 사례 | 28건 (좌표 적격 23건 · 좌표 미확인 5건) |
| 전국·인접 범위 사례 | 2건 |
| 범위 미확정 사례 | 45건 |
| 높은 신뢰도 | 64건 |
| 중간 신뢰도 | 11건 |
| 운영 상태 재검토 필요 | 75건 (보존 원본 전체도 재검토 필요) |
...
현재 대시보드의 외부화된 기준 데이터는 `baseline/v2/site.v2.json`이며, v3 작업 투영은 `data/site.v3.json`입니다. 기준 마이그레이션의 230개 사례와 17개 지역은 그대로 유지하고, 검토를 통과한 신규 사례는 `data/additions.v1.json`에서 추가합니다. 원본은 235개 레코드이며 사례 집계는 75건입니다. 보조 참고 160개는 연구 부록에 분리합니다. 자료 반영일은 2026-07-29입니다.
...
- `entries`: 계보를 보존하는 235개 원본 레코드(기준 230건 + 신규 5건); 화면의 사례 집계는 보조 참고 160개를 제외한 75건
...
- 현재 75개 사례와 160개 보조 참고 자료 모두 독립적인 사실 검토가 필요합니다.
