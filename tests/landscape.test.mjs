...
test("landscape model partitions all 230 entries into 17 interactive regions", () => {
  const model = landscapeModel(site.entries, site.regions);
  assert.equal(model.total, 230);
  assert.equal(model.regions.length, 17);
  assert.equal(model.byId.size, 17);
  assert.equal(model.regions.reduce((sum, region) => sum + region.total, 0), 230);
  assert.equal(model.nationalCategoryTotals.reduce((sum, category) => sum + category.count, 0), 230);
